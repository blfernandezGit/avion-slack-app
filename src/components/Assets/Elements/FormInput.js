const FormInput = ({type, placeholder, reference, inputName, inputLabel, search}) => {
    return (
        ...
        {search &&
            <input className="bg-transparent max-w-screen w-screen border-2 border-white focus:border-pink focus:outline-none rounded-md text-center text-white w-full" type={type} placeholder={placeholder} ref={reference}></input>
        }
        </>
    )
}
